...
export class UsuariosPersonaComponent implements OnInit {
  ...
  eliminarUsuario(usuarioRow: UsuarioAsignado) {
    this.dialog.open(EliminarModalComponent, {
          width: '400px',
          data: { mensaje: "¿Está seguro de eliminar al usuario: " + usuarioRow.usuario + " de " + this.persona.nombres + " " + this.persona.apellidoPaterno + "?" }
        }).afterClosed().subscribe(documento => {
          if (documento) {

            switch(usuarioRow.rol) {
              case'Representante': {
                this.respresentanteService.eliminar(usuarioRow.idEntidad, documento).subscribe({
                  next: () => {
                    this.mostrarMensaje('Usuario representante eliminado correctamente');
                    this.cargarUsuarios();
                  },
              ...
              break;
              case'Egresado': {
                this.egresadoService.eliminar(usuarioRow.idEntidad, documento).subscribe({
                  next: () => {
                    this.mostrarMensaje('Usuario egresado eliminado correctamente');
                    this.cargarUsuarios();
                  },
                  error: (err) => this.mostrarMensaje('Error al eliminar: ' + err.message, 'error'),
                });
              ...
              break;
              case'Administrativo':
                this.administrativoService.eliminar(usuarioRow.idEntidad, documento).subscribe({
                  next: () => {
                    this.mostrarMensaje('Usuario administrativo eliminado correctamente');
                    this.cargarUsuarios();
                  },
                  error: (err) => this.mostrarMensaje('Error al eliminar: ' + err.message, 'error'),
                });
